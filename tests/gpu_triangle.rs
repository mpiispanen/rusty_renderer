...
fn test_triangle_runs_without_crash() {
    ...
    let initialized = stdout.contains("backend initialized") || stdout.contains("Vulkan backend initialized");
    let no_panic = !stderr.contains("panic") && !stderr.contains("SIGSEGV") && !stderr.contains("segmentation fault");
    
    // 124 means timeout (expected), 0 means clean exit (also good)
    assert!(
        exit_code == 124 || exit_code == 0,
        "Triangle example crashed with exit code {} (124=timeout expected, 0=clean exit)",
        exit_code
    );
    ...
    assert!(
        no_panic,
        "Triangle example panicked or segfaulted"
    );
}
...
fn test_triangle_backend_initialization() {
    ...
    assert!(
        passed >= 4,
        "Expected at least 4 initialization steps, found {}",
        passed
    );
}
